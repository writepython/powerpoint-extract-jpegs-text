...
oopaths=('/opt/openoffice.org3/program', '/opt/openoffice.org/basis3.0/program')

OPENOFFICE_PORT = 2002
OPENOFFICE_PATH = oopaths[0]
...
class OORunner:
    ...
    def connect(self, no_startup=False):
        ...
        while n < 6:
            try:
                context = resolver.resolve("uno:socket,host=0,port=%d;urp;StarOffice.ComponentContext" % self.port)
                break
    ...
    def startup(self):
        ...
        """
        args = [OPENOFFICE_BIN,
                '-accept=socket,host=0,port=%d;urp;StarOffice.ServiceManager' % self.port,
                '-nofirststartwizard',
                '-headless',
                '-norestore',
                '-nologo',
